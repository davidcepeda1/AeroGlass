...
// genuinely have none.
const BRAVE_FALLBACK_ART_BASE64_PREFIX =
	"iVBORw0KGgoAAAANSUhEUgAAAQAAAAEACAYAAABccqhmAAAQAE";
...
function isPlaceholderCoverArt(coverArt: string | null): boolean {
	...
	return (
		payload.length === BRAVE_FALLBACK_ART_BASE64_LENGTH &&
		payload.startsWith(BRAVE_FALLBACK_ART_BASE64_PREFIX)
	);
}
...
async function resolvePaletteFor(
	coverArt: string | null,
	...
	return {
		stops: getEqualizerPalette(colors),
		peakColor: getPeakColor(colors),
	};
}
...
function App() {
	...
	const cycleEqStyle = () => {
		setEqStyle((current) => {
			const next =
				EQ_STYLES[(EQ_STYLES.indexOf(current) + 1) % EQ_STYLES.length];
	...
	const beginDiscTransition = (song: SongInfo) => {
		...
		setTimeout(async () => {
			...
			while (transitionIdRef.current === myId) {
				...
				if (
					pending &&
					!isPlaceholderTitle(pending.title) &&
					!isPlaceholderCoverArt(pending.coverArt)
				)
					break;
				...
				if (isTransitioningRef.current) {
					...
					if (song.coverArt !== paletteArtRef.current) {
						...
						resolvePaletteFor(song.coverArt).then((palette) => {
							if (paletteArtRef.current === song.coverArt)
								setEqPalette(palette);
						});
					}
				}
